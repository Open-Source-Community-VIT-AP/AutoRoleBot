...
def member_list(ctx):
    ...
    for member in server_instance:
        memberlist.append(member.name+"#"+member.discriminator) # Appends the discord tag in the desired format
        member_dictionary[member.name+"#"+member.discriminator] = member.id # Makes the key value pair
    ...
    async with ctx.typing(): # Typing animation
        ...
        for member in dictionary:
            if member in memberlist:
                user = ctx.author.guild.get_member(member_dictionary[member]) # Retrieves a discord.Member object
                role = discord.utils.get(ctx.author.guild.roles, name=dictionary[member]) # Retrieves the required role
...
async def missing(ctx):
    fields = ["DISCORD ID OF MISSING PEOPLE",]
